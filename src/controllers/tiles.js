...
tilesController.getTileZXY = function(req, res){
    ...
    var pathTile = tilesDir + '/' + deliveryCode + '/' + zoom + '/' + x + '/' + y + '.png';

    // the tile we are looking for exist?
    fs.access(pathTile, fs.constants.R_OK, function(err){
        if(err){
            var margin = 0.01;
            var tileBbox = tile2Bbox(x, y, zoom);
            var tileBboxwithMargin = addMarginsToBbox(tileBbox, margin);
            generator.generateTile(deliveryCode, propertyCode, tileBboxwithMargin, zoom, function(tile){
